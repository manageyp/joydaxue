    ...
    class << self
      ...
      def send_request(type, method, params = {})
        ...
        begin
          ...
          response = RestClient::Request.execute(:method => :post, :url => request_url,
            :payload => params, :timeout => 10, :open_timeout => 10,
            :headers => reuqest_header)
          if response.present?
            result = JSON.parse(response)
            logger.info "xinge end push: #{result}"
            [result["ret_code"], result["err_msg"]] if result.present?
          end
        rescue => e
          puts e.response
          return false
